...
import { TrendingUp, AlertTriangle, BarChart2, DollarSign } from 'lucide-react'
import {
  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
        {[
          { icon: '\ud83d\udcb0', label: 'Valor no Pipeline', val: formatBRL(totalValue), sub: opportunities.length + ' oportunidades', color: '#10b981' },
          { icon: '\ud83d\udcc8', label: 'Convers\u00e3o Geral', val: globalConv.toFixed(1) + '%', sub: 'fechados vs total', color: '#6366f1' },
          { icon: '\u26a0\ufe0f', label: 'Parados +15 dias', val: String(stalled15), sub: 'exigem a\u00e7\u00e3o imediata', color: stalled15 > 0 ? '#ef4444' : '#10b981' },
          { icon: '\ud83d\udd00', label: 'Pipelines Ativos', val: String(byPipeline.length), sub: 'SGTAX \u00b7 SG3 \u00b7 DIGITAL', color: '#8b5cf6' },
        ].map(({ icon, label, val, sub, color }) => (
          <div key={label} style={{ background: '#1a1d2e', border: '1px solid #2d3045', borderRadius: 12, padding: 20 }}>
            <div style={{ fontSize: 13, color: '#8b8fa8', marginBottom: 8 }}>{icon} {label}</div>
            <div style={{ fontSize: 26, fontWeight: 700, color }}>{val}</div>
            <div style={{ fontSize: 11, color: '#8b8fa8', marginTop: 4 }}>{sub}</div>
          </div>
        ))}
      </div>
